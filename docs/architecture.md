...
- Auto-pause media on handoff (Windows Smart Yield)

**Components:**
- `CoordinationProtocol` — Network coordination and state machine
- `MediaController` — Win32 media key control (VK_MEDIA_PLAY_PAUSE via SendInput API)

**State Machine:** `IDLE → INTENT_DETECTED → REQUESTING → CONNECTED → YIELDING → IDLE`

**Windows Smart Yield:**
- **Coordinated yield** — Pauses media BEFORE disconnecting (smooth handoff to Windows peer)
- **External takeover** — Pauses media AFTER disconnect (iPhone or manual disconnect)
- Debouncing prevents rapid disconnect spam (2-second interval)
...
**Components:**
- `BluetoothManager` — Connect/disconnect via PowerShell PnP device commands, background monitoring thread for disconnect detection
